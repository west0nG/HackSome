...
class PromptRendererTests(unittest.TestCase):
    ...
    def test_all_stage_templates_render_with_one_variable_contract(self) -> None:
        ...
        for stage_number in range(11):
            ...
            with self.subTest(stage=stage):
                ...
                self.assertIn("Assigned output target: staging/output.md", rendered.text)
                self.assertEqual(rendered.prompt_version, "1")
    ...
    def test_template_missing_contract_placeholder_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(PromptRenderError, "unexpected variables: mode"):
    ...
    def test_hashes_are_stable_across_mapping_order(self) -> None:
        first_manifest = {
            "artifacts": [
                {"artifact_type": "research", "path": "research/a/r.md"}
            ],
        ...
        second_manifest = {
            "audience_id": "audience-001",
            "artifacts": [
                {"path": "research/a/r.md", "artifact_type": "research"}
            ],
        }
    ...
    def test_required_markdown_headings_are_literal_english(self) -> None:
        for stage, headings in EXPECTED_HEADINGS.items():
            ...
            with self.subTest(stage=stage):
                ...
                self.assertIn(
                    "Use the exact English Markdown headings", rendered
                )

    def test_common_contract_rejects_relative_selection_and_forced_diversity(self) -> None:
    ...
    def test_s1_rejects_compliance_context_leakage(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            self.renderer.render(
                "S1", {**COMMON_VARIABLES, "context_manifest": leaked}
            )
    ...
    def test_each_stage_accepts_its_declared_artifact_types(self) -> None:
        allowed = {
            ...
            "S5": ("problem", "research", "verification"),
            "S6": (
                "discovery_view",
                "problem",
                "problem_gateway",
    ...
    def test_blind_reviews_reject_prior_review_paths(self) -> None:
        ...
        s9_leak = {
            "artifacts": [
                {"path": "idea-reviews/i/red-team-001.md"}
            ]
        }
...
class PromptSchemaTests(unittest.TestCase):
    def test_schema_paths_and_aliases_resolve_to_packaged_json(self) -> None:
        self.assertEqual(schema_path("s0"), PromptRenderer.schema_path("challenge-brief"))
    ...
    def test_completion_envelope_is_lightweight(self) -> None:
        ...
        self.assertEqual(
            schema["properties"]["status"]["enum"], ["completed", "empty"]
        )
