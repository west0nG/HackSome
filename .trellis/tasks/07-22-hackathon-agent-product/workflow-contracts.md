...
> 状态：v0.1 实现基线。后续根据真实 benchmark 的误杀、误放和运行故障继续修订。
...
1. 反复发生的问题默认至少需要两个独立复核的一手来源；单次后果严重的问题可以由一个能够说明具体后果的一手来源支撑。该规则是最低证据线，不是数量评分。
2. Prompt 的控制指令与可机器校验的 Markdown 标题使用英文；正文默认跟随题目语言，CLI 可以显式覆盖。
3. Prompt 模板、front matter 枚举与正文标题由版本化代码资源统一定义；改变这些契约时必须同时更新校验器与测试。
