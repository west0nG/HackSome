...
v1 不提供失败 Run 的 `resume` 命令。Hub 的完整持久化只用于复盘、状态查看和离线校验；同一个 Task 内部的基础设施重试仍可恢复准确 Codex Session。
...
Problem Gateway（每个 Problem 一个全新 Session）
    ...
    ▼
Idea Generator（每个 Problem 默认 3 个全新 Session，并行执行）
...
v1 的 `audiences` 最多包含五项。Schema 与 Hub 都会执行这一上限，避免模型输出突破第一版范围。
...
- 不设置 Problem、Idea 或 Idea Card 数量上限
