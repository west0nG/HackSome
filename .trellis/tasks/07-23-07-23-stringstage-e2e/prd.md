...
- Lead 模型只在 Goal 队列为空时运行；存在任何非终态 Goal 时 heartbeat 必须跳过
  模型调用，batch 清空后再立即唤醒 Lead。
...
- [x] 两份 initializer 被正确写入 `state/stringstage-e2e/project/reference/`。
- [x] Hub、Lead、Worker manager、Verifier manager 均成功启动。
- [x] Lead 至少创建一个真实 Goal，Worker 实际领取并对 `/project` 产生实质变更。
- [x] Verifier 对至少一个 Worker result 提交真实 PASS 或 FAIL verdict。
- [x] 若 FAIL，证据显示同一 Worker/session 被恢复；若 PASS，系统继续下一 Goal或唤醒 Lead。
- [x] `/project` 中存在足以证明项目已被实际推进的产品代码、验证结果或提交材料。
- [x] 保存一份不含凭证的 E2E 证据摘要。
