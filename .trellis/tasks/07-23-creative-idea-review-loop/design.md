...
`repairable` 只允许一次局部修复，并必须保留 `Core Mechanism`、`Intended Reaction` 与 `primary_territory_ref`。为了让该边界可机械验证，`Intended Reaction`、`Real Input, Transformation and Output`、`Parent Atoms` 三个 section body 必须与 source 保持一致，Prompt 必须把这三个不可变 section 明示给 Agent；时长、铺垫、风险和最小 Demo 的局部澄清只能写入其他 section。修复后由两个 fresh Session 再评：
