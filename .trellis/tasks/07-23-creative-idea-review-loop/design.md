...
```text
┌───────────────────────────────────────────────────────────────┐
│ RELAY ROOM              [01 项目] [02 项目] … [08 项目]       │
│ 评审者姓名 [____________________]                              │
├───────────────────────────────────────────────────────────────┤
│ 01 / 08  PROJECT TITLE                                        │
│ 一句话 Hook · exact concept ref / revision / hash              │
├───────────────────┬───────────────────┬───────────────────────┤
│ 体验与 Hook        │ 软件 / Demo       │ 分享对象 / 产物        │
│ 先看到             │ 运行入口           │ 分享触发               │
│ 观众做什么          │ 最小 Demo cut      │ 可转发物               │
│ Reveal / 余韵      │                   │                       │
├───────────────────────────────────────────────────────────────┤
...
│ 想发给谁 [____________]  action [____]                         │
│ 分享冲动 [____]          Demo 信心 [____]                      │
│ 反应 chips / 只针对本项目的自由评论                             │
└───────────────────────────────────────────────────────────────┘

┌───────────────────────────────────────────────────────────────┐
│ 02 / 08  NEXT PROJECT …（重复同一完整结构）                     │
└───────────────────────────────────────────────────────────────┘
```

关键不变量：

- 页面可以纵向堆叠多张卡并提供项目跳转，但一个 `article` 只能对应一个 exact Concept。
- 标题、三块事实、折叠原文和项目回执必须位于同一个 `article`；不得把五道或更多评审问题抽成脱离项目的共享区域。
- 三块事实不是模型分数：它们分别重排原始体验、软件 Demo 和传播信息，所有 run 文本仍用 `textContent` 渲染。
...
移动端只改变列数，不改变信息归属：三块事实按单列展开，项目回执仍在同卡内紧随其后，页面不得横向滚动。
