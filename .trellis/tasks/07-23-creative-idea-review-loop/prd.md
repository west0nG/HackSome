...
- 每个 Concept 都要说明：
  ...
  - 结果为何出人意料但又可以理解；
  - `Minimum Hackathon Demo`：团队时间内的最小 build cut、现场步骤、真实依赖、最难技术点和可验收证据；
...
**关卡：** Concept 不能只是名称、口号、功能列表、美学换皮或无法解释的 AI 效果；必须完整描述可执行的软件核心、端到端 Demo 路径和具体分享触发物。缺少必需结构是任务输出无效，不是候选被模型“拒绝”。
...
- C4H 评估分类式判断，而不是一个总体 1-10 分：
  ...
  - 30 秒路径能够抵达一个有感受的时刻；
  - 无需长篇解释即可复述 Concept；
  - 存在具体、立即、可解释的分享触发物，而不是只宣称“很 viral”。
- C4F 独立判断：
  - 核心体验由可执行软件产生；
...
- 两个独立 C6B curator 对 `software_demo_strength`、`surprise_fun_or_intrigue`、`one_sentence_clarity`、`immediate_share_trigger`、`novel_combination` 逐项输出 `pass|uncertain|fail` 和 evidence；任一 `fail` → `exclude`，全 `pass` → `include`，其余 → `hold`。不得输出总分或排序。
...
- **C3：** 从铺垫到揭示的完整路径、software-first 合格率、端到端技术路径完整率，以及具体 share artifact 覆盖率。
- **C4：** C4H/C4F 的 `pass|repairable|invalid` 分布与原因码；定制硬件和纯装置 fixture 的 false-pass、合法普通设备 I/O fixture 的 false-reject、可修复缺信息 fixture 是否只使用一次 repair。
- **C5：** 历史线索命中率、challenger 的非复制变换与 C4F 通过率、旧硬件模式再引入率、完整 C4 screen 后进入 C5W 的任务数及 token/wall-time 节省、有价值外部碰撞和引用有效性。
- **C6：** C6B categorical 维度分布、shortlist 中 `immediate_share_trigger=pass` 的占比、候选名单多样性、评审负担、一句话复述准确度、`share_impulse=immediate`、具体分享对象、`demo_confidence=yes`、困惑率、人类分歧，以及依据反馈修订的忠实度。
...
- [ ] C6A 接收 feasibility evidence 但只能做有界澄清；C6B 使用 software demo、surprise/fun、clarity、share-trigger、novelty 的 categorical evidence 透明缩减候选集，不输出 1–10 总分或排序。
...
- [ ] 多名具名评审者可以评估候选名单中的精确修订版本；每份回执记录一句话复述、`share_impulse`、`demo_confidence` 与具体分享对象，且 `immediate` 必须填写对象。
