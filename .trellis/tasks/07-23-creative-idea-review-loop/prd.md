...
为 hackathon 构建一条只产出构想的 Creative 路线，系统性探索出人意料、好玩、神秘且值得分享的概念，而不强迫它们接受 Useful 路线的需求频率与商业价值模型检验。
...
这是构想阶段对创意清晰度与分享意愿的代理指标，并非对现实世界传播力的断言。
...
- **先独立创作，再读取历史。** 当前运行的第一批 C2/C3 与初始 C4 的受控上下文、Prompt 和 parent refs 不得注入过去的 Idea；历史只能在后续有界分支中提供抽象灵感或避坑线索。v1 把这定义为可审计的上下文隔离，而不是操作系统级机密隔离。
...
- `run`、`status`、`validate` 和报告依据持久化的路线身份进行分派；`resume` 是可选的路线能力，v1 只由 Creative 实现，并严格区分 C6 业务恢复与 C7 确定性发布重放。
...
**关卡：** 简报包含正向体验目标、反目标，以及足够清晰、可指导独立探索的场景。
...
**关卡：** Concept 不能只是名称、口号、功能列表、美学换皮或无法解释的 AI 效果。

### C4. 低成本 Hook 筛选

**目的：** 在投入网络搜索和人工评审成本之前，淘汰无法形成清晰体验的 Concept。

**行为：**

- 使用全新且相互独立的评审 Session，每个 Session 只能获得 Creative Brief、约束和一个 Concept。
- 评估分类式判断，而不是一个总体 1-10 分：
  - 铺垫可以快速理解；
  - 揭示会改变受众的预期；
  - 惊喜源自机制；
  - 30 秒路径能够抵达一个有感受的时刻；
  - 无需长篇解释即可复述 Concept；
  - Concept 不依赖隐蔽的人工工作或不可能具备的权限。
- 输出 `pass`、`repairable` 或 `invalid` 之一，并为每项未通过的判断提供证据和稳定维度原因码。
- 仅当核心机制和预期反应保持不变时，最多允许一次局部修复。
- 控制器为每个 revision 写入不可变的处置记录，区分通过、进入修复、双 `invalid` 淘汰和修复后仍未解决；基础设施或协议失败不得伪装成淘汰。

**输出：** 不可变的 `CheapHookReview`、`ConceptDisposition` 产物，以及可选的 Concept 修订。

**关卡：** 只有 `pass` 的 Concept 才能进入先例研究。
...
- challenger 不能再次触发 Memory Recall；它与第一批 Concept 使用完全相同的双 reviewer C4 合同、最多一次 Hook repair 和淘汰规则。
- 只有第一批或 challenger 中通过 C4 的 Concept 才进入 C5W 联网扫描。
...
**输出：** 冻结的 `IdeaMemorySnapshot`、`MemoryInspirationPacket`、controller-owned `MemoryStageSummary`、零到两个带历史谱系的 challenger，以及每个 Hook-pass Concept 的不可变 `NoveltyScan`。
...
- v1 使用按目的分开的有界预算，而不是“整个生命周期只能改一次”：C4 最多一次 Hook 局部修复，C6 自动准备恰好一次证据驱动修订，C6 人审关闭后最多一次反馈驱动修订。
- 因而，每个 base Concept 或 memory challenger 在最坏情况下最多经历三次模型修订；每次都有不同的原因、输入边界和版本谱系。某一阶段未使用的预算不能挪到另一阶段，也不能通过重试增加次数。

**自动准备：**

- 依据每个符合条件的 Concept 的 Cheap Hook Review、Novelty Scan、相关 Memory Inspiration 和 Constraint View 进行修订，同时保留 Concept 身份与历史。
- 使用透明的分类证据自动生成有界候选名单，而不是使用模型生成的单一数值分数。
- 保留实质不同的领域，避免候选名单只是同一个构想的多个表面变体。
- 保留所有未进入候选名单的 Concept，并记录它们未进入人工批次的原因。
- 如果第一批与 memory challenger 均没有 Hook-pass Concept，或自动 shortlist 最终为空，控制器仍发布 `concept_refs=[]` 的空 `HumanReviewBatch`，写明 `skip_reason`，跳过 `waiting` 和评审页面后直接进入 C7。这个 batch 只是证明“没有待审对象”，不会删除任何 Concept、Review 或淘汰原因。

**人工策展：**

- 针对候选名单中的精确修订版本，提供完善的 HTML/CSS/JavaScript 评审页面。
- 允许多名具名队友独立评审。
- 收集准确的一句话复述、评审者是否想到具体的分享对象、轻量反应、自由文本评论，以及可选的保留/修订/拒绝或品味否决理由；长度只受明确的安全预算限制，不强迫固定评价格式。
- 支持选定的两两比较，但不要求完成所有可能的配对。
- 当前评审者提交之前，不得展示更早评审者的答案。
- 普通 reviewer 在 round 关闭前看不到 Idea Memory 来源或历史淘汰标签，避免污染 30 秒独立判断；Percy 的策展模式可以查看来源、变换说明和 copy-risk，C7 最终完整披露。
...
- 包含：
  ...
  - 未解决的分歧和推迟处理的风险；
  - 路线、Prompt、筛选和报告策略版本。
- 有效运行可以零个最终 Idea 结束，但必须写出稳定的 `zero_reason_code`，区分没有生成 Concept、所有候选未通过 Hook、自动 shortlist 为空和人类全部拒绝/否决。
...
**输出：** 成功运行生成确定性的 `creative-idea-report.md`、机器可读摘要和 `creative-memory-record.json`；失败运行生成 `creative-partial-report.md` 和对应摘要，但 v1 的 partial run 不进入未来 Idea Memory。
...
- **C1：** 与 Percy 预期反应及反目标的一致性。
- **C2：** 机制实质不同的数量，而非标题多样性。
- **C3：** 从铺垫到揭示的完整路径，以及无法解释的魔法所占比例。
- **C4：** 与人工审计的 Hook 示例相比，误通过和误拒绝的情况。
- **C5：** 历史线索命中率、challenger 的非复制变换、可见的历史失败规避、有价值的外部碰撞发现、引用有效性，以及真正不同组合的保留情况。
- **C6：** 候选名单多样性、评审负担、一句话复述准确度、自发识别分享对象、困惑率、人类分歧，以及依据反馈修订的忠实度。
...
Percy 稍后将提供真实的 hackathon 挑战用于实际评估。默认自动化测试套件保持离线；付费/在线 Codex 基准运行必须显式选择启用。
...
- [ ] C2 在不受先例锚定且不强制要求商业痛点的情况下，产出多个实质不同的创意领域。
- [ ] C3 产出版本化 Concept，包含完整的 30 秒铺垫、受众行动、揭示、机制、余韵及最小 Demo 形态。
...
- [ ] C4 独立拒绝令人困惑、仅有文案、无法实现或缺少机制的 Concept，并且最多允许一次有界的局部修复；每个处置都有稳定原因码和 review/decision 证据，修复前 revision 以带目标引用的 successor disposition 闭合。
- [ ] C5M 从当前 run 创建时冻结、hash 绑定的 completed Creative memory record 中选择线索；无历史或 `--idea-memory off` 时不调用 Recall Agent，且行为退化为原始流程。
- [ ] C5M 最多生成两个“当前 Atom × 历史线索”的 challenger，不递归读取 memory；每个 challenger 具有新身份、完整来源和变换说明，并重新通过与普通 Concept 相同的 C4/C5W。
...
- [ ] C5W 只在初始生成与可选 Memory Remix 之后联网，并记录有引用的现有成果、碰撞、陈词滥调和独特性证据。
- [ ] C6 透明地缩减候选集，同时保留组合多样性和每个被排除候选项的历史。
...
- [ ] 没有待审 Concept 时仍生成带 `skip_reason` 的空 C6 batch，不进入人工等待；所有已生成 Concept、逐项 Hook 判断和淘汰原因仍出现在 C7。
- [ ] 多名具名评审者可以评估候选名单中的精确修订版本；每份回执都记录 30 秒后的一句话复述，以及是否想到、若想到则具体想分享给谁，并允许留下自由文本反馈。
- [ ] 成对比较完全可选：评审者可以回答任意子集，也可以全部跳过，未回答的 pair 不被解释为平局、拒绝或缺失回执。
- [ ] 每个 reviewer 在首次提交前看不到他人答案；提交后的团队视图与 Percy 策展权限不会因共享链接或 cookie 混淆身份。
- [ ] 普通 reviewer 在 round 关闭前无法读取 memory provenance；Percy curator 能审查历史来源、借用/规避内容和 copy-risk，最终报告可追溯。
...
- [ ] C6 等待期间即使代码库同步更新，`resume` 仍使用 run 创建时冻结并校验的 Prompt/Schema；不支持的 contract 或被篡改资源 fail closed。
...
- [ ] Memory Record 区分 C4 机制/可理解性 caution、自动 curator 支持不足、仅因 portfolio 容量未入选、人类主观否决、merge 变换和 final 正向模式，未来 Agent 不会把这些结果压成同一种“失败”。
...
- [ ] 基准输出与一次性基线比较，并报告质量、多样性、token/时间成本、评审负担及不完整的覆盖情况；记录 memory policy/snapshot hash，并支持 memory-on/off 离线消融。
