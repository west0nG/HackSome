...
> **Git Autonomy Policy (this project).** `commit`, `push`, `branch`, and `worktree` are **pre-authorized** — perform them autonomously, no per-action confirmation prompt. Commit granularity is at the AI's discretion (coherent units, not one-per-file, not one giant dump). After work commits land, **push** so the remote is never left behind. Create a branch / git `worktree` for a task when isolation helps, off the task's `base_branch`, without asking. Two hard limits stay: (1) never sweep **unrecognized** dirty files (paths you did not edit this session) into a commit — report and leave them dirty; (2) an explicit user instruction for the current turn ("don't commit yet" / "我自己来" / "manual") overrides this default. Authority: user standing instruction (2026-06-27).
...
5. **Commit autonomously — no confirmation prompt** (Git Autonomy Policy above). Run `git add <files>` + `git commit -m "<msg>"` for each batch in order; granularity at your discretion. Only AI-edited files go in — **unrecognized** dirty files are never swept in silently: leave them dirty and report once in your turn summary. Report the result (don't ask first):
   ```
   Committed (in order):
     1. <message>  [<n> files]
     2. <message>  [<n> files]
   Left dirty (not mine this session): <file>, <file>
   ```

6. **Push autonomously**: after the work commits land, `git push` (use `-u origin <branch>` on the first push of a new branch). Don't leave the branch ahead of origin at the end of the turn.

7. **User override always wins**: if the user said "don't commit yet" / "我自己来" / "manual" for this work, honor that and skip — autonomy is the default, not a mandate.

**Rules**:
- No `git commit --amend` anywhere — three-stage three-commit flow (work commits → archive commit → journal commit).
- Push is autonomous (this project): push after the work commits, and again after `/finish-work` bookkeeping (archive + journal commits), so the remote is never left behind.
- Unrecognized dirty files are never committed silently — report and leave them.
- Branch / worktree creation for a task needs no confirmation (off `base_branch`); use judgment on when isolation is worth it.
